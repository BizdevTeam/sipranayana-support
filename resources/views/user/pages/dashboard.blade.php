                        ...
                        <div class="timeline">
                            ...
                            @foreach ($reportLogs as $log)
                                @php
                                    ...
                                    $isCurrentUser = $log->user && $log->user->id === Auth::id();
                                    $isAdmin = $log->user && $log->user->role === 'admin';
                                    $bubbleColor = $isAdmin ? '#fff3cd' : ($isCurrentUser ? '#d4edda' : '#e3e3e3'); // Admin: Kuning, User: Hijau, Lainnya: Abu-abu
                                    $textColor = $isAdmin ? '#856404' : ($isCurrentUser ? '#155724' : '#333'); // Admin: Coklat, User: Hijau, Lainnya: Hitam
                                    $iconColor = $isAdmin ? 'bg-warning' : ($isCurrentUser ? 'bg-success' : 'bg-secondary');
                                @endphp
                        
                                <!-- Label Tanggal -->
                                @if ($logDate !== $currentDate)
                                    <div class="time-label">
                                        <span class="bg-success text-white px-2 py-1 rounded">{{ $logDate }}</span>
                                    </div>
